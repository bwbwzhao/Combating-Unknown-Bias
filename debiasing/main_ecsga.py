...
if __name__=='__main__':
    ...
    parser.add_argument('--cfg', default='./configs/celeba.yaml', type=str)
    parser.add_argument('--dataset_tag', default='CelebA', type=str)
    parser.add_argument('--main_tag', default='tmp', type=str)
    parser.add_argument('--bmodel', default='det_peer_eta0.9', type=str)
    ...
    scores = torch.load(
        os.path.join(cfg['log_dir'], cfg['dataset_tag'], cfg['bmodel']+'_seed%d'%(args.seed), "model_b.th")
    )['scores']
    # avg_bcscores = 1. - scores[:,-1] # 
    # avg_bcscores = 1. - scores[:,7] # 
    avg_bcscores = 1. - scores.mean(dim=1) #
    writer.add_pr_curve(
        'ensemble', 
    ...
    tau = 0.8
    e_index = avg_bcscores<tau
    h_index = avg_bcscores>=tau

    hard_set = copy.deepcopy(train_dataset)
    hard_set.attr = hard_set.attr[h_index]
    hard_set.data = hard_set.data[h_index]
    easy_set = copy.deepcopy(train_dataset)
    easy_set.attr = easy_set.attr[e_index]
    easy_set.data = easy_set.data[e_index]
    ...
    for epoch in range(cfg['main_num_epoch']):
        ...
        for _ in range(len(train_dataset)//cfg['main_batch_size']):
            ...
            easy_mask = (label==label)
            easy_mask[:hard_bs] = False
            hard_mask = (label==label)
            hard_mask[hard_bs:] = False

            logit = model(data)
            loss_per_sample = F.cross_entropy(logit, label, reduction='none')

            loss_weight = torch.ones_like(loss_per_sample)
            pos_grad = 1. - torch.exp(-loss_per_sample).detach()
            frac = pos_grad[hard_mask].sum() / (pos_grad[easy_mask].sum() * cfg['gamma'] + 1e-10)
            loss_weight[easy_mask] = frac
