...
if __name__=='__main__':
    ...
    for epoch in range(50):
        ...
        for _ in range(len(train_dataset)//cfg['main_batch_size']):
            ...
            p = torch.exp(-loss_per_sample.detach())
            for c in range(attr_dims[0]):
                p[label==c] /= p[label==c].max()

            # confidence based 
            loss_weight = torch.zeros_like(loss_per_sample)
            loss_weight[p > cfg['eta']] = 1.0

            loss = (loss_weight * loss_per_sample).mean()
