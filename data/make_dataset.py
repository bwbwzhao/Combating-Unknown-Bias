...
def make_colored_mnist(data_dir, skewed_ratio, severity):
    ...
    for split in ["train", "valid"]:
        ...
        # dataset = EMNIST(mnist_dir, split='letters', train=(split == "train"), download=True)
        # used_idx = (dataset.targets <= 11) & (dataset.targets!=9)
        # dataset.data = dataset.data[used_idx]
        # dataset.data = T.functional.hflip(T.functional.rotate(dataset.data, -90))
        # dataset.targets = dataset.targets[used_idx]
        # dataset.targets = dataset.targets - 1
        # idx_9 = (dataset.targets==9)
        # idx_10 = (dataset.targets==10)
        # dataset.targets[idx_9] -= 1
        # dataset.targets[idx_10] -= 1
...
def make(make_target):
    ...
    # for skewed_ratio in [0.0001]:
    #     for severity in [4]:
            if make_target == "colored_mnist":
                make_colored_mnist('./debias_datasets/mnist/', skewed_ratio=skewed_ratio, severity=severity)
    
            if make_target == "cifar10_type0":
                make_corrupted_cifar10(
                    './debias_datasets/cifar10/',
                    corruption_names=[
                        "Snow",
                        "Frost",
                        "Fog",
